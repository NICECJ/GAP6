...
int rtsolpr_prrtadv_build_probe(struct ethhdr *eth,
    ...
    icmp6->icmp6_type = 133; /* Router Solicitation */
    return 0;
}
...
int rtsolpr_prrtadv_parse_response(const uint8_t *buffer,
    ...
    if (icmp6_hdr->icmp6_type != 134) /* Router Advertisement */
        return 0;
    ...
    *target_ip = ip6_hdr->ip6_src;
    ...
    uint32_t ap_id;
    memcpy(&ap_id, (uint8_t*)icmp6_hdr + sizeof(struct icmp6_hdr), sizeof(uint32_t));
    *prefix_index = ntohl(ap_id) >> 8;
...
protocol_t rtsolpr_prrtadv_protocol = {
    .name = "rtsolpr_prrtadv",
    .build_probe = rtsolpr_prrtadv_build_probe,
    .parse_response = rtsolpr_prrtadv_parse_response
};

/* automatic registration */
__attribute__((constructor))
static void register_rtsolpr_prrtadv()
{
    register_protocol(&rtsolpr_prrtadv_protocol);
}