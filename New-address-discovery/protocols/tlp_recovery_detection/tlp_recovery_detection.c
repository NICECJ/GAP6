#include "tlp_recovery_detection.h"
...
int tlp_recovery_detection_build_probe(struct ethhdr *eth,
                                      struct ip6_hdr *ip6,
                                      struct tcphdr *tcp,
                                      int index) {
...
int tlp_recovery_detection_parse_response(const uint8_t *buffer,
...
protocol_t tlp_recovery_detection_protocol = {
    .name = "tlp_recovery_detection",
    .build_probe = tlp_recovery_detection_build_probe,
    .parse_response = tlp_recovery_detection_parse_response
};

/* automatic registration */
__attribute__((constructor))
static void register_tlp_recovery_detection()
{
    register_protocol(&tlp_recovery_detection_protocol);
}