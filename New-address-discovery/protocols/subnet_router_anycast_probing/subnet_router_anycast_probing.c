#include "subnet_router_anycast_probing.h"
...
int subnet_router_anycast_probing_build_probe(struct ethhdr *eth,
                                             struct ip6_hdr *ip6,
...
int subnet_router_anycast_probing_parse_response(const uint8_t *buffer,
    ...
    if (icmp6_hdr->icmp6_type != 129)  // Echo Reply
        return 0;
...
protocol_t subnet_router_anycast_probing_protocol = {
    .name = "subnet_router_anycast_probing",
    .build_probe = subnet_router_anycast_probing_build_probe,
    .parse_response = subnet_router_anycast_probing_parse_response
};

/* automatic registration */
__attribute__((constructor))
static void register_subnet_router_anycast_probing()
{
    register_protocol(&subnet_router_anycast_probing_protocol);
}