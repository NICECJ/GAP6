...
int rtsolpr_to_prrtadv_build_probe(struct ethhdr *eth,
                                  struct ip6_hdr *ip6,
...
int rtsolpr_to_prrtadv_parse_response(const uint8_t *buffer,
    ...
    if (icmp6_hdr->icmp6_type != ND_ROUTER_ADVERT || icmp6_hdr->icmp6_code != 0)
        return 0;
...
protocol_t rtsolpr_to_prrtadv_protocol = {
    .name = "rtsolpr_to_prrtadv",
    .build_probe = rtsolpr_to_prrtadv_build_probe,
    .parse_response = rtsolpr_to_prrtadv_parse_response
};

/* automatic registration */
__attribute__((constructor))
static void register_rtsolpr_to_prrtadv()
{
    register_protocol(&rtsolpr_to_prrtadv_protocol);
}