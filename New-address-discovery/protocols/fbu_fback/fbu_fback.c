...
int fbu_fback_build_probe(struct ethhdr *eth,
                         struct ip6_hdr *ip6,
...
int fbu_fback_parse_response(const uint8_t *buffer,
    ...
    if (icmp6_hdr->icmp6_type != 136)  // Binding Acknowledgment
        return 0;

    // TODO: Add BADF option verification here
    // This would require parsing ICMPv6 options
...
protocol_t fbu_fback_protocol = {
    .name = "fbu_fback",
    .build_probe = fbu_fback_build_probe,
    .parse_response = fbu_fback_parse_response
};

/* automatic registration */
__attribute__((constructor))
static void register_fbu_fback()
{
    register_protocol(&fbu_fback_protocol);
}